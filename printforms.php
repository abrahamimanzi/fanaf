    ...
    if($participant_data->payment_state == "Confirmed" || $participant_data->payment_state == "Pending"){
        ...
        class PDF extends FPDF{
            ...
            function Header()
            {
                ...
                $this->Image(DN.'/img/logo.png',30,40,160);
                // Arial bold 15
                ...
                $this->Cell(0,65,'42eme Assemblee Generale Annuelle de la FANAF',0,0,'R');
                $this->Cell(0,95,'fanaf2018.org',0,0,'R');
...
// Instanciation of inherited class
        ...
        $pdf->Cell(0,0,'Cher '.$participant_data->firstname.' '.$participant_data->lastname.' '.$participant_data->othername,0,0,'C');
        
        $pdf->SetY(185); 
        $pdf->SetFont('Arial','',10);
        // $pdf->Cell(0,0,'To pay by bank transfer, kindly use the following details:',0,2,'C');
        $pdf->Cell(0,0,'Pour payer par virement bancaire, veuillez utiliser les informations suivantes:',0,2,'C');
        ...
        $pdf->Cell(0,0,'',1,1,'L');
            ...
            $pdf->Cell(40,25,'Raison / reference de paiement:',0,1,'R');
            ...
            $pdf->Cell(40,25,'Nom du compte',0,1,'R');
            ...
            $pdf->Cell(40,25,'Nom de banque',0,1,'R');
            ...
            $pdf->Cell(40,25,'Nom de la filiale',0,1,'R');
            ...
            $pdf->Cell(40,25,'USD Compte No.',0,1,'R');
            ...
            $pdf->Cell(40,25,'RWF Compte No.',0,1,'R');
            ...
            $pdf->Cell(40,25,'Adresse de la banque',0,1,'R');
...
KN 4 Ave, Kigali – Rwanda',0,1,'L');
        ...
        $pdf->Cell(0,0,'N\'oubliez pas d\'utiliser votre identifiant d\'enregistrement comme moyen de paiement',0,2,'C');

        $pdf->SetY(505); 
        $pdf->SetFont('Arial','',10);
        // $pdf->Cell(0,0,'reference as it is used to identify your payment. Payment should be processed',0,2,'C');
        $pdf->Cell(0,0,'de reference car elle est utilisee pour identifier votre paiement. Le paiement devrait etre traite',0,2,'C');

        $pdf->SetY(520); 
        $pdf->SetFont('Arial','',10);
        // $pdf->Cell(0,0,'within 5 working days of submitting the registration form. Please allow 3-4',0,2,'C');
        $pdf->Cell(0,0,'dans les 5 jours ouvrables suivant la soumission du formulaire d\'inscription. S\'il vous plait permettez 3-4',0,2,'C');

        $pdf->SetY(535); 
        $pdf->SetFont('Arial','',10);
        // $pdf->Cell(0,0,'working days for bank transfer to be cleared. Registration is considered',0,2,'C');
        $pdf->Cell(0,0,'jours ouvrables pour que votre virement bancaire s\'effectue. L\'inscription est consideree',0,2,'C');

        $pdf->SetY(550); 
        $pdf->SetFont('Arial','',10);
        // $pdf->Cell(0,0,'complete only once the payment has been received. For further information',0,2,'C');
        $pdf->Cell(0,0,'complete seulement une fois que le paiement a ete recu. Pour plus d\'informations',0,2,'C');

        $pdf->SetY(565); 
        $pdf->SetFont('Arial','',10);
        // $pdf->Cell(0,0,'regarding the payment, contact us on '.EMAIL1,0,2,'C');
        $pdf->Cell(0,0,' contactez-nous sur '.EMAIL1,0,2,'C');
        // $pdf->SetY(215); 
