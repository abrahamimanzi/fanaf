<?php // http://localhost/fanaf/accommondationpayment/316
...
if(Input::checkInput('id','get','1')){
    ...
    switch($request){
        ...
        default:
            $participant_ID = $_id;
            $participantTable = new \Accommadation();
            $participantTable->selectQuery("SELECT * FROM `accommadation` WHERE `ID`=? ORDER BY `ID` DESC LIMIT 1",array($participant_ID));
		...
		case 'payment-new':
            ...
            switch($request){
                ...
                default:
                    ...
                    $discount = 0;
                    $participantTable = new \Accommadation();
                    $participantTable->selectQuery("SELECT * FROM `accommadation` WHERE `ID`=? AND `payment_state`='Pending' OR `payment_state`='Cancelled' ORDER BY `ID` DESC LIMIT 1",array($participant_code));
                    ...
                    // }
                    // $gross = $net_total;
                    $net_total = $participant_data->price;
                    $gross = $participant_data->price;
                    ...
                    $participantTable->update(array(
                        'price'=>$member_other_data->unit_price,
                        'amount'=>$member_other_data->amount,
                        'discount'=>$member_other_data->discount,
                        'currency'=>$currency_str,
                    ),$participant_data->ID);
                    ...
                    $queryData = array(
                        ...
                        'vpc_Version' => 1,
                        // 'vpc_ReturnURL' => 'http://127.0.0.1/bk-payment/return_url.php',
                        'vpc_ReturnURL' => (DN.'/accommondationsuccess/'.$participant_data->ID.'/success'),
         ...
         <?php 
            switch($request){
                case 'group':
                    ...
                    break;
                default:
                    include 'views/register/accommondationpayment-form.php';
                    break;
            }?>
        </section>
