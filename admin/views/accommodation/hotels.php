              ...
              <div>
                ...
                <div class="box">
                    ...
                    <?php
                        ...
                        $accommodationTable->selectQuery("SELECT * FROM `hotels`",$sql_val_array);
                    ...
                    ?>
                      <h3 class="box-title">Hotels list <span style='color:#01aef0'><?=$accommodationApprovalNumber?></span></h3>
                      ...
                      <div class="table-responsive">
                        <table class="table no-margin">
                          <thead>
                            <form method="post">
                              <tr>
                                <th><button type="submit" name="sort-id">Hotel. ID</button></th>
                                <th><button type="submit" name="sort-name">Hotel Name</button></th>
                                <th><button type="submit" name="sort-name">Hotel Email</button></th>
                                <th><button type="submit" name="sort-category">Distance</button></th>
                          ...
                          <tbody>
                            ...
                            $seconds = \Config::get('time/seconds');
                              $accommodationTable = new Hotels();
								...
								$accommodationTable->selectQuery("SELECT* FROM `hotels` WHERE `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['accommodation_sort']}",$sql_val_array);
                            ...
                            if(Input::checkInput('request','post',1)){
                                ...
                                $accommodationTable->selectQuery("SELECT * FROM `hotels` WHERE `added_date` >= '$from_date' AND `added_date` <= '$to_date'");
                              ...
                              $accommodationTable = new Hotels();
                              $accommodationTable->selectQuery("SELECT * FROM `hotels` WHERE `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['accommodation_sort']} LIMIT {$offsetNumber},{$rowsLimit}",$sql_val_array);
								...
								if($accommodationTable->count()){
									...
									foreach($accommodationTable->data() as $accommodation_data){
                                      ...
                                      <tr class="row_layout">
                                        ...
                                        <td>
                                            <span data-toggle="modal" data-target="#myModal_<?=$participant_data->ID?>" ><?php echo $accommodation_data->distance;?></span>
                                        </td>
