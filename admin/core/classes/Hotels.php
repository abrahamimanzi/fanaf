...
//USER CREATE AN ACCOUNT
	public function insert($fields = array()){
		...
		if(!$this->_db->insert('hotels', $fields)){
			throw new Exception("There was a problem to add your Hotel.");
		}
	}
...
// USER DATA UPDATE
	public function update($fields = array(),$id = null){
		if(!$this->_db->update('hotels',$id,$fields)){
...
// FIND USER
	public function find($user = null,$limit = null){
		if($user){
			...
			if(is_numeric($user)){
				$field = 'ID';
				$data = $this->_db->get('hotels', array($field, '=', $user),$limit);
				if($data->count()){
	...
	public function select($sql = null){
		$data = $this->_db->query("SELECT* FROM `hotels` {$sql}");
		if($data->count()){
